...
def run_mask(camera_name,runs):
    object = []
    hand = []
    object.append({'vertices':runs.obj_verts*1000,'faces':runs.obj_faces})
    hand.append({'vertices':runs.hand_verts,'faces':runs.closed_faces,'joints':runs.hand_joints,'pose':runs.hand_pose,'beta':runs.hand_beta})
    # renderer for object mesh
    # note the mesh_scale, ContactPose object models are in units of mm 
    object_renderer = rutils.DepthRenderer(object[0], cp.K(camera_name), camera_name, mesh_scale=1e-3)
    ...
    hand_renderers.append(None)
    #else:
    renderer = rutils.DepthRenderer(hand[0], cp.K(camera_name), camera_name)
    hand_renderers.append(renderer)
    return {'object': object_renderer, 'hands': hand_renderers}
