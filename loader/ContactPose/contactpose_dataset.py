...
def get_object_names(p_num, intent, ignore_hp=True):
  ...
  sess_dir = osp.join(osp.dirname(__file__), '../../', 'HOinter_data/ContactPose', 'contactpose_data', sess_dir)
...
def get_intents(p_num, object_name):
  ...
  for ins in ('use', 'handoff'):
    ...
    sess_dir = osp.join(osp.dirname(__file__), '../../', 'HOinter_data/ContactPose', sess_dir, object_name)
...
class ContactPose(object):
  ...
  def __init__(self, p_num, intent, object_name, mano_pose_params=15,
    ...
    self.data_dir = osp.join(osp.dirname(__file__), '../../', 'HOinter_data/ContactPose', p_id, object_name)
    ...
    oM = np.loadtxt(osp.join(osp.dirname(__file__), '../../', 'HOinter_data/ContactPose',
